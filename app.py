...
with col3:
    if st.button("🔄 새로고침", use_container_width=True, help="데이터가 업데이트됐을 때 눌러주세요 (캐시 삭제)"):
        st.cache_data.clear()
        st.session_state.data = None
        st.success("캐시 삭제됨 / 다시 불러오기를 눌러주세요.")
        st.rerun()
...
if st.session_state.data:
    ...
    **사용법** 💡  
    ...
    한 번 입력된 링크는 한 시간 단위로 캐싱되니 
    
    데이터가 업데이트 되었을 경우 반드시! 새로고침 버튼을 눌러 주셔야 합니다
    """)
